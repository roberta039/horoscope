...
with st.sidebar:
    st.header("👤 Informații Personale")
    
    # Nume și prenume
    col_name1, col_name2 = st.columns(2)
    with col_name1:
        first_name = st.text_input("Prenume", value="", placeholder="Introdu prenumele")
    with col_name2:
        last_name = st.text_input("Nume", value="", placeholder="Introdu numele")
    ...
    st.header("📍 Locație Naștere")
    
    # Țară și oraș
    country = st.text_input("Țară", value="", placeholder="ex: România")
    city = st.text_input("Oraș", value="", placeholder="ex: București")
    
    selected_capital = st.selectbox("Sau alege o capitală:", list(capitale_lumii.keys()))
    ...
    st.subheader("Coordonate geografice:")
    
    col_lat, col_lon = st.columns(2)
    with col_lat:
        latitude = st.number_input("Latitude", value=44.4268, format="%.6f", 
                                 help="Introdu latitudinea în grade zecimale")
    with col_lon:
        lon_input = st.text_input("Longitude", value="26.1025",
                                help="Formate acceptate: 26.1025 sau 26°6'")
    
    # Buton pentru calcul
    calculate_button = st.button("🎯 Calculează Horoscop", type="primary", use_container_width=True)
...
if calculate_button:
    try:
        ...
        if not first_name or not last_name:
            st.error("⚠️ Te rog completează numele și prenumele!")
            st.stop()
        
        if not country or not city:
            st.warning("ℹ️ Pentru o analiză mai precisă, completează țara și orașul de naștere.")
        ...
        try:
            if selected_capital != "Selectează o capitală":
                ...
                city = selected_capital  # Actualizează orașul cu capitala selectată
            else:
        ...
        # Afișare date introduse
        st.header(f"📊 Horoscop pentru {first_name} {last_name}")
        
        col_info1, col_info2 = st.columns(2)
        with col_info1:
            st.write(f"**📅 Data nașterii:** {birth_date.strftime('%d %B %Y')}")
            st.write(f"**🕐 Ora nașterii:** {birth_time.strftime('%H:%M')}")
            st.write(f"**👤 Gen:** {gender}")
        
        with col_info2:
            st.write(f"**🌍 Locație:** {city}, {country}")
            st.write(f"**📍 Coordonate:** Lat {latitude:.4f}°, Lon {longitude:.4f}°")
        ...
        # Layout cu coloane pentru rezultate
        col1, col2 = st.columns(2)
        ...
        with col1:
            ...
            for planet, data in positions.items():
                ...
                speed_symbol = "⚡" if data['speed'] > 0 else "🐢" if data['speed'] < 0 else "⏸️"
                st.write(f"**{planet_name}**: {data['sign']} {data['degrees']:.1f}° {speed_symbol}")
        ...
        if aspect_list:
            for aspect in aspect_list:
                orb_color = "🟢" if aspect['orb'] < 2 else "🟡" if aspect['orb'] < 5 else "🟠"
                st.write(f"{orb_color} **{aspect['planet1']}** {aspect['aspect']} **{aspect['planet2']}** "
                        f"({aspect['angle']:.1f}°, orb: {aspect['orb']:.1f}°)")
        else:
            st.info("ℹ️ Nu s-au găsit aspecte majore în orbul specificat")
        
        # Informații Ascendent și MC
        ascendant_sign_index = int(ascmc[0] / 30)
        ascendant_degrees = ascmc[0] % 30
        
        mc_sign_index = int(ascmc[1] / 30)
        mc_degrees = ascmc[1] % 30
        
        col_asc, col_mc = st.columns(2)
        with col_asc:
            st.info(f"**🌅 Ascendent**: {signs[ascendant_sign_index]} {ascendant_degrees:.1f}°")
        with col_mc:
            st.info(f"**🏔️ Medium Coeli**: {signs[mc_sign_index]} {mc_degrees:.1f}°")
        
    except Exception as e:
        st.error(f"❌ Eroare la calcul: {e}")

# Afișare când aplicația este deschisă fără calcul
else:
    st.markdown("""
    ## 👋 Bun venit în Aplicația de Astrologie!
    
    ### 📝 Cum funcționează:
    1. **Completează informațiile personale** în bara laterală
    2. **Introdu datele tale de naștere** (data, ora, locația)
    3. **Alege o capitală** sau introdu coordonate manual
    4. **Apasă butonul 'Calculează Horoscop'**
    
    ### 🌟 Ce vei obține:
    - 🪐 **Pozițiile planetare** în zodiac
    - 🏠 **Casele astrologice** 
    - 🔗 **Aspectele** dintre planete
    - 🌅 **Ascendentul** și Medium Coeli
    
    ### 📍 Formate acceptate pentru longitudine:
    - **Grade zecimale**: `26.1025`
    - **Grade-minute**: `26°6'` sau `26 6`
    
    ***💫 Completează datele în bara laterală și descoperă-ți horoscopul!***
    """)

# Footer
st.markdown("---")
st.markdown("*Aplicație dezvoltată cu Swiss Ephemeris și Streamlit* | *Date astronomice precise*")
